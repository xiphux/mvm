 ...
 *
 *  $Id: gui.cpp 210 2005-10-15 04:18:35Z xiphux $
 */
#include "gui.h"
#include "../mvm.h"
...
void mvm::gui::gui::drawdatapaths()
{
	 ...
	 */
	if (!VM->dp->IDmux->get_signal1()&&!VM->dp->IDmux->get_signal2())
		attron(A_BOLD);
	 ...
	 */
	if (VM->dp->ex_mem->RegW->get()==VM->dp->id_ex->RS->get())
		attron(A_BOLD);
	 ...
	 */
	if (VM->dp->mem_wb->RegW->get()==VM->dp->id_ex->RS->get())
		attron(A_BOLD);
...
void mvm::gui::gui::drawcontrolpaths()
{
	 ...
	 * IF AND -> IF MUX
	 */
	if (VM->dp->temp_ctrl1&VM->dp->jump)
		attron(A_BOLD);
